...
def SoApps():
    SCOfile = open('appwxstations.txt')
    print('Collecting Data...\n\n')
    ...
    print("Length y " + str(jf))
    for b in x:
        plt.text((x[p])-0.2,(y[p])+0.1,plotdata[p],color='red',size=6)
        print("Temp " + str(p))
        plt.text((x[p])-0.2,(y[p])-0.1,plotdew[p],color='green',size=6)
        p = p + 1
    ax.set(xlim=(-85.62, -77.83), ylim=(33.37, 39.49))
    plt.xlim(-85.62, -77.83)
    plt.ylim(33.37, 39.49)
    #plt.text(-3,-0.7,"Valid: " + Time_7 + "     " + "Source: NCSCO",ha='center',size=12,color='white')

    sf = shp.Reader("SoAppsCounties.shp")
    ...
    sf = shp.Reader("SoAppsStates.shp")
